...
#executable="time ../../source/godunov"
executable="../../source/bin/solve "
...
#echo setenv OMP_NUM_THREADS 12 >> run.sh
echo export OMP_NUM_THREADS=12 >> run.sh
while read -u 4 inputs
  do	
	...
	if [ "$a" = "1" ]; then
		...
		echo cd $directory\; $executable $input_file.input \>\& $input_file.log\; cd ../ >> run.sh  
            echo "" >> run.sh
	fi
done
