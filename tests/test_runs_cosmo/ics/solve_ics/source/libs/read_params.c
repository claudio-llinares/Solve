...
void init_keywords(struct params *par, char keywords[50][1000], void *values[50], char type[50][10], int *found)
{
  ...
  strcpy(keywords[0],  "file_in");
  strcpy(keywords[1],  "file_out");
  strcpy(keywords[2],  "box");
  strcpy(keywords[3],  "grid");
  strcpy(keywords[4],  "grid_p");
  strcpy(keywords[5],  "omegam");
  strcpy(keywords[6],  "omegar");
  strcpy(keywords[7],  "omegal");
  strcpy(keywords[8],  "hsmall");
  strcpy(keywords[9],  "z_init");
  strcpy(keywords[10],  "z_end");
  strcpy(keywords[11], "nthreads");
  strcpy(keywords[12], "Nseed");
  strcpy(keywords[13], "nsteps_nbody");
  strcpy(keywords[14], "sign_delta");
  keywords[15][0] = '\0';
  
  values[0]  = &par->file_in;
  ...
  values[9]  = &par->z_init;
  values[10] =  &par->z_end;
  values[11] = &par->nthreads;
  values[12] = &par->Nseed;
  values[13] = &par->nsteps_nbody;
  values[14] = &par->sign_delta;
  ...
  strcpy(type[10],  "double");
  strcpy(type[11], "int");
  strcpy(type[12], "long");
  strcpy(type[13],  "int");
  strcpy(type[14],  "double");
  ...
  while( keywords[i][0] != '\0')
    {
      if(!found[i])
	{
	  fprintf(stderr, "Keyword %s not present in input file.  Aborting.\n", keywords[i]);
	  exit(0);
	}
      i++;
    }
	  ...
	  if(!strcmp(type[i], "char"))
	    ...
	    if(!strcmp(type[i], "int"))
	      *((int *)values[i]) = atoi(val);
	      ...
	      if(!strcmp(type[i], "long"))
		*((int *)values[i]) = atol(val);
	      else
