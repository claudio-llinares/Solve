 ...
 *  See also ``differential_operators_mm.c``.
**/
#include<stdio.h>
#include <math.h>
#include <stdlib.h>
...
void gs_node_mm(double rho, double source_phi, double source_chi, double ***phi, double ***chi, double cte_poisson_over_a, double aa0, double a2M2, double h, double fourh, int i, int j, int k, int ip1, int im1, int jp1, int jm1, int kp1, int km1)
{
  ...
  phi[i][j][k] = ( nb_sum - pow2(h)*(-a2M2*chi[i][j][k] + cte_poisson_over_a*rho + source_phi) ) / (6.0-a2M2*pow2(h));
  ...
  chi[i][j][k] = ( sum1 - pow2(h)*(-a2M2*phi_old + cte_poisson_over_a*rho + source_chi) ) / sum2;
